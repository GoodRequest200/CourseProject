...
const loadDepartments = async () => {
  try {
    ...
    if (!res.ok) throw new Error('Не удалось загрузить цехи')
...
const loadEvent = async () => {
  try {
    ...
    selectedDepartments.value = (data.departments ?? []).map((d) => d.id)
  } catch (e) {
    error.value = e.message || 'Не удалось загрузить событие'
  }
}

onMounted(async () => {
  if (!isAdmin.value) {
    error.value = 'У вас нет прав для редактирования событий'
    return
  }
...
const submit = async () => {
  ...
  if (!isAdmin.value) {
    error.value = 'Только администратор может редактировать события'
    return
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(text || 'Не удалось обновить событие')
    }

    const updated = await res.json()
    success.value = 'Событие обновлено'
    router.push(`/events/${updated.id}`)
  } catch (e) {
    error.value = e.message || 'Не удалось обновить событие'
  } finally {
    loading.value = false
  }
}
...
<template>
  <section class="page">
    ...
    <p v-if="!isAdmin" class="error">У вас нет прав для редактирования событий</p>
